//! Auto-save and crash-recovery for CutDeck projects.
//!
//! Design:
...
fn autosave_path(cutdeck_dir: &PathBuf, project_id: &str) -> PathBuf {
    cutdeck_dir.join(format!("{}.autosave.json", project_id))
}

/// Returns the main project file path for a given project_id.
fn project_path(cutdeck_dir: &PathBuf, project_id: &str) -> PathBuf {
    cutdeck_dir.join(format!("{}.json", project_id))
}

/// Returns the cutdeck_dir (shared with project.rs storage).
async fn get_cutdeck_dir(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    let app_dir = app
        .path()
        .app_data_dir()
...
pub async fn auto_save_project(
    ...
    let cutdeck_dir = get_cutdeck_dir(&app).await?;
    let target_path = autosave_path(&cutdeck_dir, &project_id);

    tokio_fs::write(&target_path, &content)
        .await
        .map_err(|e| format!("自动保存失败: {e}"))?;

    tracing::debug!("[CutDeck] Autosaved project {} to {:?}", project_id, target_path);
    Ok(())
}
...
pub async fn clear_autosave(
    ...
    let cutdeck_dir = get_cutdeck_dir(&app).await?;
    let autosave = autosave_path(&cutdeck_dir, &project_id);

    if autosave.exists() {
        tokio_fs::remove_file(&autosave)
            .await
            .map_err(|e| format!("清除自动保存失败: {e}"))?;
        tracing::debug!("[CutDeck] Cleared autosave for project {}", project_id);
    }
    Ok(())
}
...
pub async fn list_recoverable_projects(
    ...
    let mut entries = tokio_fs::read_dir(&cutdeck_dir)
        .await
...
pub async fn recover_autosave(
    ...
    let cutdeck_dir = get_cutdeck_dir(&app).await?;
    let autosave = autosave_path(&cutdeck_dir, &project_id);
    let main_file = project_path(&cutdeck_dir, &project_id);
    ...
    tracing::info!("[CutDeck] Recovered project {} from autosave", project_id);
    Ok(content)
}
...
mod tests {
    ...
    fn test_autosave_path_format() {
        let dir = std::path::PathBuf::from("/data/cutdeck");
        let id = "my-project";
        let result = autosave_path(&dir, id);
        assert_eq!(result, std::path::PathBuf::from("/data/cutdeck/my-project.autosave.json"));
    }
    ...
    fn test_project_path_format() {
        ...
        assert_eq!(result, std::path::PathBuf::from("/data/cutdeck/my-project.json"));
    }
