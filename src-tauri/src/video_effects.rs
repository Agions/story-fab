...
fn filter_description(filter: &VideoFilter) -> String {
    match filter {
        ...
        VideoFilter::Contrast(v) => format!("对比度 {v:.1f}"),
        VideoFilter::Brightness(v) => format!("亮度 {v:.1f}"),
        VideoFilter::Saturation(v) => format!("饱和度 {v:.1f}"),
        VideoFilter::Blur(r) => format!("模糊 {r:.1f}px"),
        VideoFilter::Sharpen(s) => format!("锐化 {s:.1f}"),
        VideoFilter::Vignette => "暗角 (Vignette)".to_string(),
        VideoFilter::Grain(g) => format!("胶片颗粒 {g:.2f}"),
