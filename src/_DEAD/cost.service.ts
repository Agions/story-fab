...
*报告由 CutDeck 成本追踪服务生成*
  ...
  saveToStorage(): void {
    try {
      localStorage.setItem('cutdeck_cost_records', JSON.stringify(this.records));
      localStorage.setItem('cutdeck_cost_budget', JSON.stringify(this.budget));
    } catch (error) {
  ...
  loadFromStorage(): boolean {
    try {
      const records = localStorage.getItem('cutdeck_cost_records');
      const budget = localStorage.getItem('cutdeck_cost_budget');
