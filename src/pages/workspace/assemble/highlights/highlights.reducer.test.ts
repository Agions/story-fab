...
describe('highlightsReducer', () => {
  describe('SET_HIGHLIGHTS', () => {
    it('sets highlights array', () => {
      const next = highlightsReducer(makeState(), { type:'SET_HIGHLIGHTS', payload: mockHighlights });
    ...
    it('sets empty highlights array', () => {
      ...
      const next = highlightsReducer(prev, { type:'SET_HIGHLIGHTS', payload: [] });
      expect(next.highlights).toEqual([]);
    });

    it('preserves other state fields', () => {
      const prev = makeState({ loading: true, error: 'old error' });
      const next = highlightsReducer(prev, { type:'SET_HIGHLIGHTS', payload: mockHighlights });
  ...
  describe('SET_DETECTED', () => {
    it('sets detected to true', () => {
      const next = highlightsReducer(makeState(), { type:'SET_DETECTED', payload: true });
      expect(next.detected).toBe(true);
    });

    it('sets detected to false', () => {
      const prev = makeState({ detected: true });
      const next = highlightsReducer(prev, { type:'SET_DETECTED', payload: false });
  ...
  describe('SET_LOADING', () => {
    it('sets loading to true', () => {
      const next = highlightsReducer(makeState(), { type:'SET_LOADING', payload: true });
      expect(next.loading).toBe(true);
    });

    it('sets loading to false', () => {
      const prev = makeState({ loading: true });
      const next = highlightsReducer(prev, { type:'SET_LOADING', payload: false });
  ...
  describe('SET_ERROR', () => {
    it('sets error message', () => {
      const next = highlightsReducer(makeState(), { type:'SET_ERROR', payload: 'Network error' });
      expect(next.error).toBe('Network error');
    });

    it('clears error to null', () => {
      const prev = makeState({ error: 'Some error' });
      const next = highlightsReducer(prev, { type:'SET_ERROR', payload: null });
  ...
  describe('SET_THRESHOLD', () => {
    it('sets threshold value', () => {
      const next = highlightsReducer(makeState(), { type:'SET_THRESHOLD', payload: 2.5 });
      expect(next.threshold).toBe(2.5);
    });

    it('preserves other state fields', () => {
      const prev = makeState({ topN: 5, loading: true });
      const next = highlightsReducer(prev, { type:'SET_THRESHOLD', payload: 3.0 });
  ...
  describe('SET_TOPN', () => {
    it('sets topN value', () => {
      const next = highlightsReducer(makeState(), { type:'SET_TOPN', payload: 5 });
      expect(next.topN).toBe(5);
    });

    it('preserves other state fields', () => {
      const prev = makeState({ threshold: 2.0 });
      const next = highlightsReducer(prev, { type:'SET_TOPN', payload: 20 });
  ...
  describe('START_DETECT', () => {
    it('sets loading to true', () => {
      const next = highlightsReducer(makeState(), { type:'START_DETECT', payload: undefined });
      expect(next.loading).toBe(true);
    });

    it('clears error to null', () => {
      const prev = makeState({ error: 'Previous error' });
      const next = highlightsReducer(prev, { type:'START_DETECT', payload: undefined });
      expect(next.error).toBeNull();
    });
  ...
  describe('DETECT_SUCCESS', () => {
    it('sets highlights from action', () => {
      const next = highlightsReducer(makeState(), { type:'DETECT_SUCCESS', payload: mockHighlights });
      expect(next.highlights).toEqual(mockHighlights);
    });

    it('sets detected to true', () => {
      const prev = makeState({ detected: false });
      const next = highlightsReducer(prev, { type:'DETECT_SUCCESS', payload: mockHighlights });
      expect(next.detected).toBe(true);
    });
  ...
  describe('DETECT_FAILURE', () => {
    it('sets error message', () => {
      const next = highlightsReducer(makeState(), { type:'DETECT_FAILURE', payload: 'API timeout' });
      expect(next.error).toBe('API timeout');
    });

    it('sets loading to false', () => {
      const prev = makeState({ loading: true });
      const next = highlightsReducer(prev, { type:'DETECT_FAILURE', payload: 'API timeout' });
      expect(next.loading).toBe(false);
    });

    it('preserves other state fields', () => {
      const prev = makeState({ highlights: mockHighlights, detected: true, threshold: 2.0, topN: 5 });
      const next = highlightsReducer(prev, { type:'DETECT_FAILURE', payload: 'fail' });
      expect(next.highlights).toEqual(mockHighlights);
      expect(next.detected).toBe(true);
      expect(next.threshold).toBe(2.0);
      expect(next.topN).toBe(5);
  ...
  describe('FINISH_DETECT', () => {
    it('sets loading to false', () => {
      const prev = makeState({ loading: true });
      const next = highlightsReducer(prev, { type:'FINISH_DETECT', payload: undefined });
      expect(next.loading).toBe(false);
    });
