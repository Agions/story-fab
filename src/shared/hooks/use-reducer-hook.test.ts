...
import { useReducerHookFactory } from './use-reducer-hook';
...
describe('useReducerHookFactory', () => {
  it('returns initial state', () => {
    const { result } = renderHook(() => useReducerHookFactory(reducer, initial));
  ...
  it('memoizes state reference across renders', () => {
    const { result, rerender } = renderHook(() => useReducerHookFactory(reducer, initial));
